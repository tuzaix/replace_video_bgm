...
from crypto_tool import machine_code, verify_license
...
def _resource_path(*parts: str) -> Path:
    ...
    parts : str
        ...
        _resource_path("gui", "wechat", "admin1.png").
...
def _license_is_ok() -> bool:
    ...
    """
    try:
        lic_path = _default_license_path()
...
def _show_no_nvidia_dialog(app: QtWidgets.QApplication) -> None:
...
def _show_license_failure_dialog(app: QtWidgets.QApplication) -> None:
    ...
    try:
        ...
        wechat_dir = _resource_path("gui", "wechat")
        qr_candidates = []
        try:
...
def run_preflight_checks(app: QtWidgets.QApplication) -> bool:
    ...
    try:
        ...
        has_nv = False
    if not has_nv:
        _show_no_nvidia_dialog(app)
        return False

    # 2) License/authorization check
    try:
        lic_ok = _license_is_ok()
    except Exception:
        lic_ok = False
    if not lic_ok:
        _show_license_failure_dialog(app)
        return False
